...
- Flow Governance context actions for starting release, feature, and hotfix branches from `main`; `Start New Release` verifies and, after confirmation, synchronizes an out-of-sync current `main` before opening its form. Bugs start from a release or feature using a Bug ID, short name, and description, and tasks start from a feature using a Dev Task number, short name, and description. Newly created branches offer optional confirmed publication.
