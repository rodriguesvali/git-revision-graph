...
- Flow Governance context actions for starting release, feature, hotfix, task, and bug branches use a fetch-first source preflight. Untracked sources are checked on a configured remote and offer confirmed publication when absent. Release, feature, and hotfix require `main` to match its upstream; task and bug accept ahead-only feature/release sources, synchronize behind-only sources after confirmation, and block divergent sources for manual reconciliation. Newly created branches offer optional confirmed publication. Tasks and bugs retain their source branch and expose guarded Promotion PR Context back to that exact feature or release, including remote-target verification and confirmed source publication or push.
- Flow Governance Pull Request handoff to supported GitHub, Azure DevOps, GitLab.com, AWS CodeCommit, and Google Secure Source Manager remotes, with remote-source preflight, local/remote production synchronization and release/hotfix ancestry blocking, confirmed publish/push assistance, divergence protection, and local `sync/*` equalization of release or feature branches from `main` or an active release without automatic push. Before equalization, both the target and selected origin are fetched and checked: ahead-only local work is preserved, behind-only branches can be safely updated after confirmation, and divergence blocks the operation. Each sync branch retains its original release or feature target and exposes guarded Promotion PR Context after its merge is completed. GitHub, Azure DevOps, and GitLab receive branch-aware creation URLs; CodeCommit opens its repository Pull Requests area, while Secure Source Manager opens the documented repository page for manual Pull Request navigation. PR title and description remain editable and, together with new-release descriptions, offer optional user-triggered AI improvement through an available GitHub Copilot language model. Delivery and release prompts summarize bounded `README.md`, `CHANGELOG.md`, and approved `project-context` changes, while bug, hotfix, and synchronization prompts use a bounded code diff after sensitive paths are excluded. Optional context is reduced automatically against the available models' token limits while preserving form content.
