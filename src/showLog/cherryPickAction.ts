...
export async function cherryPickShowLogCommits(
  ...
  if (isCherryPickInProgress(repository)) {
    return handleCherryPickInProgress(repository, services, executeGit);
  }
  ...
  try {
    ...
    if (isEmptyCherryPickError(error)) {
      return handleEmptyCherryPick(repository, services, executeGit);
    }
...
async function handleCherryPickInProgress(
  ...
  return abortCurrentCherryPick(repository, services, executeGit);
}
