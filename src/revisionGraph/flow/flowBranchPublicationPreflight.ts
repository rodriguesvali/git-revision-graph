...
export type FlowBranchPublicationStatus =
  | 'ready'
  | 'unpublished'
  | 'unpushed'
  | 'remote-ahead'
...
export interface FlowBranchPublication {
  readonly status: FlowBranchPublicationStatus;
...
export type FlowBranchPublicationGitExecutor = (
  repositoryPath: string,
  args: readonly string[],
  options?: GitExecOptions
) => Promise<GitExecResult>;

export type FlowRemoteBranchCommit =
  | { readonly status: 'found'; readonly commit: string }
  | { readonly status: 'missing' }
  | { readonly status: 'unknown'; readonly detail: string };

export async function loadFlowRemoteBranchCommit(
  repository: Repository,
  remoteName: string,
  branchName: string,
  execGit: FlowBranchPublicationGitExecutor = execGitWithResult
): Promise<FlowRemoteBranchCommit> {
  try {
...
export async function checkFlowBranchPublication(
  repository: Repository,
  remoteName: string,
  sourceRefName: string,
  execGit: FlowBranchPublicationGitExecutor = execGitWithResult
): Promise<FlowBranchPublication> {
  try {
    const remoteBranch = await loadFlowRemoteBranchCommit(repository, remoteName, sourceRefName, execGit);
    ...
    const comparison = await execGit(
      repository.rootUri.fsPath,
      ['rev-list', '--left-right', '--count', '--end-of-options', `${sourceRefName}...${remoteBranch.commit}`],
      GIT_EXEC_REMOTE_PROFILE
    );
    ...
    return {
      status: classifyFlowBranchPublication(localAhead, remoteAhead),
      remoteName,
      sourceRefName,
      localAhead,
      remoteAhead
...
export function classifyFlowBranchPublication(
  localAhead: number,
  remoteAhead: number
): Exclude<FlowBranchPublicationStatus, 'unpublished' | 'unknown'> {
