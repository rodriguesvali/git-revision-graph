...
const REVISION_GRAPH_MESSAGE_AUTHORIZATION_POLICIES: RevisionGraphMessageAuthorizationPolicyMap = {
  ...
  'preview-flow-form': { repositoryScoped: true, isAllowed: (message, state) =>
    message.repositoryPath === state.repositoryPath && isRevisionGraphMessageAllowedForState({ ...message.action, description: '' }, state) },
...
function isAllowedFlowStartSourceKind(
  ...
  if (branchKind === 'feature') {
    return sourceKind === 'main' || sourceKind === 'release';
  }
