function formatWebviewShortDate(value: unknown): string {
...
function formatWebviewTooltipDate(value: unknown, fallbackText: string): string {
  const text = typeof value === 'string' ? value.trim() : '';
  if (!text) {
    return fallbackText;
  }
  ...
  return parsed.hasTime
    ? `${formatWebviewRelativeDate(parsed.date, new Date())} (${absolute})`
    : absolute;
}

function parseWebviewDisplayDate(text: string): { readonly date: Date; readonly hasTime: boolean } | undefined {
  ...
  if (dateOnlyMatch) {
    ...
    return Number.isNaN(date.getTime()) ? undefined : { date, hasTime: false };
  }
...
function formatWebviewAbsoluteDate(date: Date, hasTime: boolean): string {
...
function formatWebviewRelativeDate(date: Date, now: Date): string {
...
function renderCopyHashIconButton(
  ...
  return `<button class="${escapeWebviewDisplayAttribute(buttonClassName)}" type="button" title="Copy Hash" aria-label="Copy Hash" ${
    escapeWebviewDisplayAttribute(actionAttributeName)
  }="${escapeWebviewDisplayAttribute(actionName)}" data-commit-hash="${escapeWebviewDisplayAttribute(commitHash)}">${
    renderCopyHashIcon()
  }</button>`;
}

function escapeWebviewDisplayAttribute(value: unknown): string {
  return String(value || '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
function renderCopyHashIcon(): string {
