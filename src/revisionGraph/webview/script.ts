...
export function renderRevisionGraphScript(options: RenderRevisionGraphScriptOptions): string {
  return `<script nonce="${options.nonce}">${renderRevisionGraphScriptMessageBuilders()}${renderRevisionGraphScriptBootstrap(options)}${renderRevisionGraphScriptInteractions()}${renderRevisionGraphScriptGraphLogic()}${renderRevisionGraphScriptLayout()}
  </script>`;
}
