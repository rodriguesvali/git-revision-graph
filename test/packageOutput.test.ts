...
test('production build cleans compiled output before TypeScript runs', () => {
  ...
  assert.equal(
    ...
    'tsc -p ./tsconfig.webview.api.json && tsc -p ./tsconfig.webview.dom.json && tsc -p ./tsconfig.webview.status.json && tsc -p ./tsconfig.webview.loading.json && tsc -p ./tsconfig.webview.toolbar-busy.json && tsc -p ./tsconfig.webview.search.json && tsc -p ./tsconfig.webview.search-ui.json && tsc -p ./tsconfig.webview.search-highlights.json && tsc -p ./tsconfig.webview.relationship-highlights.json && tsc -p ./tsconfig.webview.primary-path.json && tsc -p ./tsconfig.webview.messages.json && tsc -p ./tsconfig.webview.state.json && tsc -p ./tsconfig.webview.host-state.json && tsc -p ./tsconfig.webview.state-model.json && tsc -p ./tsconfig.webview.json'
  );
});
