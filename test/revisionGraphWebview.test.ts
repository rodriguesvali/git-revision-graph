...
test('renders grouped graph context menus', () => {
  ...
  assert.match(html, /createRevisionGraphWebviewDelayedActionScheduler\(window, TOOLTIP_SHOW_DELAY_MS\)/);
  assert.match(html, /referenceTooltipShowScheduler\.schedule\(refElement, \(\) => showReferenceTooltip\(refElement\)\);/);
  ...
  assert.match(html, /function scheduleHideReferenceTooltip\(\) \{\s*cancelShowReferenceTooltip\(\);[\s\S]*?referenceTooltipHideScheduler\.schedule\(referenceTooltip, hideReferenceTooltip\);/s);
...
test('Flow previews debounce edits, ignore old replies and render host text safely', () => {
  ...
  preview.update({ ...action, name: '2.2' }, element);
  assert.equal(element.textContent, '<img src=x> release/2.1', 'keep the last preview visible while editing');
  assert.equal(element.getAttribute('aria-busy'), 'true');
  preview.update({ ...action, name: '2.3' }, element);
  assert.equal(timers.size, 1, 'rapid edits share a single pending update');
  assert.equal(element.textContent, '<img src=x> release/2.1');
  preview.receive({ ...messages[1], status: 'ready', text: 'Stale preview' });
  assert.equal(element.textContent, '<img src=x> release/2.1');
  preview.reset();
  ...
  assert.equal(element.textContent, 'Loading operation preview…', 'reopening must not reuse an old form preview');
...
test('creates explicit Flow AI field requests with stable ids and cancellation messages', () => {
  ...
  assert.deepEqual(messages, [
    {
      type: 'improve-flow-branch-text', requestId: 1, sourceRefName: 'feature/payment-summary',
      branchKind: 'task', branchName: '4312-rounding-copy',
      text: 'Adjust the payment rounding copy'
    },
    { type: 'cancel-flow-ai-text', requestId: 1, surface: 'task', field: 'description' }
  ]);
});
