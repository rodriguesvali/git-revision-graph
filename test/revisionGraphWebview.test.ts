...
test('renders structural commit actions for compare and branch creation', () => {
  ...
  assert.match(html, /flowBranch\.kind === 'release'[\s\S]*?Start New Feature[\s\S]*?createRevisionGraphPrepareStartFlowBranchMessage\(target, 'feature'\)[\s\S]*?Start New Package[\s\S]*?createRevisionGraphPrepareStartFlowBranchMessage\(target, 'package'\)[\s\S]*?Start New Bug[\s\S]*?createRevisionGraphPrepareStartFlowBranchMessage\(target, 'bug'\)/);
