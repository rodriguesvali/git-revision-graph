...
test('renders structural commit actions for compare and branch creation', () => {
  ...
  assert.match(html, /flowBranch\.kind === 'hotfix' \|\| flowBranch\.kind === 'sync' \|\| flowBranch\.kind === 'task'[\s\S]*?getFlowProductionBranchName\(\)[\s\S]*?getFlowPullRequestTargets\(target\.name\)\[0\]\?\.targetRefName[\s\S]*?Promotion PR Context[\s\S]*?postCopyFlowPullRequestContext\(target\.name, pullRequestTargetName\)/);
