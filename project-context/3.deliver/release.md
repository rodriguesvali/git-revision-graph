...
- Release cycle status: `1.6.3` was opened on 2026-08-01 as a patch release. Current scope is package
  ...
  also exposes guarded exact-commit checkout and local tag creation. Packaging and publication remain
...
- Show Log single-commit context menus now expose `Checkout to this` and `Create Tag`. The host
  resolves only loaded commit hashes, reuses existing checkout confirmation and tag validation, and
  serializes both operations through the repository mutation coordinator. Checkout retains raw-commit
  semantics and can produce detached HEAD; tag creation remains local until explicitly pushed.
