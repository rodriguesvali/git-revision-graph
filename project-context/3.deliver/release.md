...
Status: `1.6.3` automated and manual validation complete; packaging pending
...
- Publication evidence reconciliation: the exact Marketplace timestamp, VSIX filename, checksum,
  ...
  and hosted `macos-latest` result were not supplied and are not inferred. A local current-candidate
  Extension Host E2E attempt rebuilt the extension successfully on 2026-08-01, but VS Code `1.131.0`
  could not start because the execution environment has no X server or `DISPLAY`; no E2E assertion
  ran and the infrastructure limitation is not recorded as a product failure.
...
Status: Automated and manual validation complete; packaging pending
Opened: 2026-08-01
